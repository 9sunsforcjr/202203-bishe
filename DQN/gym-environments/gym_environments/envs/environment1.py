...
# 标记
    ...
    for i, j in G.edges():
        ...
        # 扩容，由200ODU0改为1000ODU0
        G.get_edge_data(i, j)["capacity"] = float(200)
        # G.get_edge_data(i, j)["capacity"] = float(1000)
...
class Env1(gym.Env):
  ...
  """
    ...
    def make_step(self, state, action, demand, source, destination):
        ...
        while (j < len(currentPath)):
            self.graph_state[self.edgesDict[str(currentPath[i]) + ':' + str(currentPath[j])]][0] -= demand #链路容量与demand相减

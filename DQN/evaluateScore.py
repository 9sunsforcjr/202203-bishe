...
def exec_dqn_model_episodes(experience_memory, env_dqn, agent):
    ...
    while iter_episode < len(experience_memory):
        if new_episode:
            ...
            new_state, reward, done, new_demand, new_source, new_destination = env_dqn.make_step(state, action, demand,
                                                                                                 source, destination)
            rewardAdd = rewardAdd + reward
            state = new_state
            if done:
...
if __name__ == "__main__":
    ...
    f = open('DQNevaluateScore.txt', 'a')

    for model_id in range(1, 502):
        reward_total = 0
        ...
        for count in range(10):
            ...
            for ep_num in range(NUMBER_EPISODES):
                ...
                filename = 'NSFNET-realdemand' + str(ep_num) + '.txt'
                with open(filename, 'r') as f:
                    while True:
                        ...
                        experience_memory.append((ep_num, D_tmp, p_tmp, E_tmp))
            ...
            # print(experience_memory[0])
            rewards_dqn = exec_dqn_model_episodes(experience_memory, env_dqn, dqn_agent)
            reward_total = reward_total + np.mean(rewards_dqn)

        reward_total= reward_total / 10
        ff = open('NSFNET-result' + '.txt', 'a')
        ff.write(str(model_id))
        ff.write(str(','))
        ff.write(str(reward_total))
        ff.write('\n')
        ff.close()
    #

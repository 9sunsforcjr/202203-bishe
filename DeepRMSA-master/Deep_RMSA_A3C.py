...
# 构造初始化字典函数，lambda:defaultdict(lambda:None)返回了一个默认空索引对应值为None的字典，然后这个字典又是linkmap字典的空键默认值 linkmap{空键：‘字典={‘None’}’，空键：‘字典={‘None’}’...}
# 构造二级字典里面是个元组
linkmap[1][2] = (0, 1050)
linkmap[2][1] = (3, 1050)
linkmap[1][3] = (1, 1500)
...
# traffic distrition, when non-uniform traffic is considered 不均匀的交通流量
# 这是个路由路径表？值是流量
...
model2_flag = 0 # number of future time slots to look into 啥
...
# we do not input ttl
x_dim_p = NODE_NUM*2 + k_path*( 2*M + 3 + model2_flag*3) # For each path: 1) FS needed, 2) starting index and size of M available FS-blocks, 3) average size, total available FS on the path
# 维度=1x(1+2*节点数+候选路径数量*(2*选前几个可用的FS+3))，model2_flag*3没用
x_dim_v = NODE_NUM*2 + k_path*( 2*M + 3 + model2_flag*3) #
num_layers = 5
layer_size = 128
regu_scalar = 1e-4 # 啥

...
for ii in range(1, NODE_NUM * NODE_NUM + 1):  # NODE_NUM*NODE_NUM import precalculated paths (in terms of path_links)
    ...
    if i != j:
        for k in range(N):
            ...
            Candidate_Paths[i][j][
                k] = temp_path  # note, if there are less than N paths for certain src-dest pairs, then the last a few values of temp_num equate to '0'
fp.close()
...
def update_slot_map_for_committing_wp(slot_map, current_wp_link, current_fs, current_fe, slot_map_t,
...
def update_slot_map_for_releasing_wp(slot_map, current_wp_link, current_fs,
...
if __name__ == "__main__":
    ...
    for a in range(1000):
        ...
        for ex in range(ITERATION):
            score = 0
            filename = 'NSFNET-demand'+str(ex)+'.txt'
            SRC = []
            DST = []
            DEMAND=[]
            index = 0
            with open(filename, 'r') as f:
                while True:
                    ...
                    p_tmp, E_tmp, D_tmp = [int(i) for i in lines.split(',')]  # 将整行数据分割处理，如果分割符是空格，括号里就不用传入参数，如果是逗号， 则传入‘，'字符。
            ...
            # lambda_intervals = 1 / lambda_req  # average time interval between request   1/12
            ...
            while req_id < num_req_measure:
                ...
                if True:  # If is used just for the sake of convenience...
                    ...
                    # current_src = temp[0]
                    # current_dst = temp[1]
                    current_src=SRC[index]
                    current_dst = DST[index]
                    index=index+1
                    # 修改带宽
                    # current_bandwidth = np.random.randint(25, 101)
                    current_bandwidth =DEMAND[index]
            ...
            sum_util=sum_util+np.mean(resource_util)

            sum_s=sum_s+score/64
    sum_s=sum_s/1000
    sum_util = sum_util / 1000
    print('//////////////////////////')
    print('score=', sum_s / ITERATION)
    print('Mean Resource Utilization =', sum_util/ITERATION)